...
describe("ListingCard", () => {
  ...
  it("não renderiza dados pessoais do anunciante", () => {
    render(
      <MemoryRouter>
        <ListingCard listing={mockListing} onSelect={vi.fn()} />
      </MemoryRouter>
    ...
    expect(screen.queryByText(/bloco/i)).not.toBeInTheDocument();
  ...
  it("não exibe preço quando price é undefined", () => {
    render(
      <MemoryRouter>
        <ListingCard
          listing={{ ...mockListing, price: undefined }}
          onSelect={vi.fn()}
        />
      </MemoryRouter>
    );
